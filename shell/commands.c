...
void cmd_bingofetch(const char *args) {
    (void)args;

    /* ── Renkler (Catppuccin Mocha) ── */
    uint32_t col_logo  = 0x89B4FA;  // mavi
    uint32_t col_label = 0x45475A;  // gri
    uint32_t col_os    = 0x89B4FA;
    uint32_t col_kernel= 0xA6E3A1;
    uint32_t col_cpu   = 0xF9E2AF;
    uint32_t col_ram   = 0xF5C2E7;
    uint32_t col_shell = 0x89DCEB;
    uint32_t col_user  = 0xFAB387;
    ...
    *(uint32_t*)(cpu_vendor+8) = ecx;
    ...
    /* RAM (CMOS) */
    outb(0x70, 0x30); unsigned char lo = inb(0x71);
    outb(0x70, 0x31); unsigned char hi = inb(0x71);
    unsigned int total_ram_mb = ((unsigned int)((hi << 8) | lo) / 1024) + 1;

    extern unsigned int boot_ticks;
    unsigned int uptime_sec = boot_ticks / 3000; /* yaklaşık */

    extern char current_user[16];
    extern char system_hostname[16];

    /* ── ASCII Art (sizin logonuz) ── */
    const char *logo[] = {
        "*%=                                                                                                 ",
        "                                             .*%@@#                                                 ",
        "                                           =@@@@@@@#                                                ",
    ...
    const char *labels[] = {"OS", "KERNEL", "CPU", "RAM", "UPTIME", "SHELL", "USER", NULL};

    /* Değerleri string olarak hazırla (elle) */
    char values[7][64];

    /* OS */
    const char *os_str = "BlexOS";
    int pos = 0; while (os_str[pos]) { values[0][pos] = os_str[pos]; pos++; } values[0][pos] = '\0';

    /* Kernel */
    const char *kern_str = "nightly f109514";
    pos = 0; while (kern_str[pos]) { values[1][pos] = kern_str[pos]; pos++; } values[1][pos] = '\0';

    /* CPU */
    char cpu_str[64];
    pos = 0;
    const char *vend = cpu_vendor;
    while (*vend) cpu_str[pos++] = *vend++;
    cpu_str[pos++] = ' '; cpu_str[pos++] = '(';
    cpu_str[pos++] = 'F'; cpu_str[pos++] = 'a'; cpu_str[pos++] = 'm'; cpu_str[pos++] = 'i'; cpu_str[pos++] = 'l'; cpu_str[pos++] = 'y';
    cpu_str[pos++] = ' ';
    int f = cpu_family;
    if (f >= 10) { cpu_str[pos++] = '0' + (f/10); f %= 10; }
    cpu_str[pos++] = '0' + f;
    cpu_str[pos++] = ','; cpu_str[pos++] = ' ';
    cpu_str[pos++] = 'M'; cpu_str[pos++] = 'o'; cpu_str[pos++] = 'd'; cpu_str[pos++] = 'e'; cpu_str[pos++] = 'l';
    cpu_str[pos++] = ' ';
    int m = cpu_model;
    if (m >= 10) { cpu_str[pos++] = '0' + (m/10); m %= 10; }
    cpu_str[pos++] = '0' + m;
    cpu_str[pos++] = ')';
    cpu_str[pos] = '\0';
    pos = 0; while (cpu_str[pos]) { values[2][pos] = cpu_str[pos]; pos++; } values[2][pos] = '\0';

    /* RAM */
    char ram_str[16];
    pos = 0;
    int ram = total_ram_mb;
    if (ram >= 1000) { ram_str[pos++] = '0' + (ram/1000); ram %= 1000; }
    if (ram >= 100)  { ram_str[pos++] = '0' + (ram/100);  ram %= 100;  }
    if (ram >= 10)   { ram_str[pos++] = '0' + (ram/10);   ram %= 10;   }
    ram_str[pos++] = '0' + ram;
    ram_str[pos++] = ' '; ram_str[pos++] = 'M'; ram_str[pos++] = 'B';
    ram_str[pos] = '\0';
    pos = 0; while (ram_str[pos]) { values[3][pos] = ram_str[pos]; pos++; } values[3][pos] = '\0';

    /* Uptime */
    char uptime_str[16];
    pos = 0;
    int upt = uptime_sec;
    if (upt >= 100) { uptime_str[pos++] = '0' + (upt/100); upt %= 100; }
    if (upt >= 10)  { uptime_str[pos++] = '0' + (upt/10);  upt %= 10;  }
    uptime_str[pos++] = '0' + upt;
    uptime_str[pos++] = ' '; uptime_str[pos++] = 's';
    uptime_str[pos] = '\0';
    pos = 0; while (uptime_str[pos]) { values[4][pos] = uptime_str[pos]; pos++; } values[4][pos] = '\0';

    /* Shell */
    const char *shell_str = "BlexSH nightly-f109514";
    pos = 0; while (shell_str[pos]) { values[5][pos] = shell_str[pos]; pos++; } values[5][pos] = '\0';

    /* User */
    char user_str[32];
    pos = 0;
    const char *u = current_user;
    while (*u) user_str[pos++] = *u++;
    user_str[pos++] = '@';
    const char *h = system_hostname;
    while (*h) user_str[pos++] = *h++;
    user_str[pos] = '\0';
    pos = 0; while (user_str[pos]) { values[6][pos] = user_str[pos]; pos++; } values[6][pos] = '\0';

    /* ── Renk dizileri ── */
    uint32_t label_colors[] = { col_label, col_label, col_label, col_label, col_label, col_label, col_label };
    uint32_t value_colors[] = { col_os, col_kernel, col_cpu, col_ram, col_ram, col_shell, col_user };

    /* ── Ekrana bas ── */
    putchar('\n');
    int info_idx = 0;
    for (int i = 0; logo[i] != NULL; i++) {
        /* Logo satırını mavi renkte yaz */
        tty_print_color(logo[i], col_logo);

        /* 60 karaktere tamamla (padding) */
        int len = str_len(logo[i]);
        int pad = 60 - len;
        if (pad < 0) pad = 0;
        for (int j = 0; j < pad; j++) tty_putchar(' ');

        /* Eğer bilgi satırı kaldıysa yaz */
        if (info_idx < 7) {
            tty_print_color(labels[info_idx], label_colors[info_idx]);
            tty_putchar(' ');
            tty_print_color(values[info_idx], value_colors[info_idx]);
            info_idx++;
        }
        tty_putchar('\n');
    }
...
/* kbd.h exposes kbd_shifts and kbd_us layout tables */
...
/*
 * cmd_layout() – Show current keyboard info
 * Kullanım: layout
 * US QWERTY düzeni (tek desteklenen) hakkında bilgi verir.
 *
 * Shift combos:
 *   Shift+4 = $, Shift+8 = *, Shift+' = ", etc.
 */
void cmd_layout(const char *args) {
    (void)args;
    print_str_color("Keyboard layout: US QWERTY\n", VGA_ATTR(COL_GREEN, COL_BLACK));
    print_str("Shift combos: Shift+4 = $, Shift+' = \", etc.\n");
    print_str("Caps Lock, Ctrl, Alt supported.\n");
}
...
command_t command_table[] = {
    ...
    {"layout",   cmd_layout,   "Show keyboard layout info"},
    {"bingofetch", cmd_bingofetch, "Show system info with ASCII logo (fastfetch style)"},
};

int command_count = 41;
